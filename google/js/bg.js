...
function MZK_getJSON_DATA(API, send_data, _rcallback) {
    ...
    $.post(
            mzk_config.base_domain + API, send_data,
            function (data) {
                if (_rcallback) {
                    _rcallback(data);
                }
...
chrome.webRequest.onAuthRequired.addListener(
        function (details) {
            ...
            if (details.isProxy === true) {
                ...
                var username = "";
                var password = "";
                if(typeof mzk_select_server_info.p_user !== "undefined"){
...
class Mzk_Kepplive_Service{
    ...
    static sync_online_data(syncdata){
        ...
        if(mzk_select_server_info.line_mode == "smart" && mzk_user_info.is_vip && mzk_config.device_name != "firefox") {
            if(syncdata.smart_pac_update != mzk_pac_config.smart_online_template.last_update || !mzk_pac_config.smart_online_template.data) Mzk_Kepplive_Service.sync_online_pactpl();
        }
    }
...
function KeepLive_Session() {
    ...
    MZK_getJSON_DATA("chromeext/index/getsession/token/"+mzk_user_token+"?appver=" + mzk_config.app_ver, {geoip_info:ipdata}, function (data) {
        if (data.result == "ok") {
            ...
            if(mzk_select_server_info.vip_level > mzk_user_info.vip_level){
                get_default_server(function(){
                    open_vpn();
                });
            }
            ...
            if(data.auto_test_server.length > 0) { // 处理测速服务器数据
                Mzk_Kepplive_Service.sync_testdomain(data.auto_test_server);
            }
        } else {
            // 假如检测服务器此 token 失效, 则自动注销当前用户
